...
int main(int argc, char *argv[]) {
	...
	listen: while(1) {
		//while(1) { //Sender side will time out eventually so recvfrom shouldn't get stuck
		n = recvfrom(receiver_sockfd, buffer, PACKET_SIZE, 0, (sockaddr*) &serv_addr, &serv_addr_size);
		//	if(n != -1)
		//		break;
		//}
		if(n < 0 || random_prob() < lossProbability) {//Go back to listen, let server side timeout
			printf("Packet was lost.\n");
			goto listen;
		}
		else if (random_prob() < corruptionProbability) {
			...
			if(Packet->seq_num == currentExpectedSeqNumber) { //Received the packet we're expecting
				...
				else { //more packets to receive: ACK current received packet.
				...
				printf("Wrong packet, dropping packet number %lu\n", Packet->seq_num);
				goto listen;
			}
			/*else {//Wrong seq number, do nothing let server timeout
		...
		//Assume program doesn't need to detect if initial file request was lost (just rerun client if no response)
		//Thus no need to resend initial file req or ACKS (lost ack = server timeout and resend)
